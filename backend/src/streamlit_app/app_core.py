...
from history_store import init_db, save_search, list_recent_searches, clear_history
from auth_store import init_auth_db, log_event

csv_path = os.path.join(BACKEND_DIR, "final_data.csv")
embeddings_path = os.path.join(BACKEND_DIR, "embeddings")
encoding = "latin1"
history_db_path = os.path.join(APP_DIR, "search_history.db")
auth_db_path = os.path.join(APP_DIR, "auth.db")
...
class App:
    ...
    def run_with_progress(self, label: str, fn: Callable[..., Any], *args, **kwargs) -> Any:
        ...
        try:
            while not future.done():
                ...
                pct = min(int((elapsed / 300.0) * 100), 99)
                bar.progress(pct, text=str(label))
                time.sleep(1.0)
    ...
    def execute_rag_search(self, query: str, top_k: int, domain: str) -> None:
        ...
        result = self.run_with_progress(
            "RAG Answering...", _rag_worker, self.resources, query, min(int(top_k), 5), domain
        )
    ...
    def execute_more_like_this(self, index: int, domain: str) -> None:
        ...
        cache_key = (index, str(domain).strip().lower(), tuple(sorted(i for i in all_result_indices if i is not None)))
        ...
        filtered_results = [
            ...
            if item.get("index") not in all_result_indices and float(item.get("score", 0.0)) > 0.70
        ]
    ...
    def perform_search(self) -> None:
        ...
        if search_method_value == "Semantic Search":
            ...
            self.execute_rag_search(query_text, top_k_value, selected_domain_value)
