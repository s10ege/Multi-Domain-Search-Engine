...
def init_sessions_table(db_path: str) -> None:
    """Create the persistent session-token table if it does not exist."""
    with sqlite3.connect(db_path) as conn:
        conn.execute("""
...
def create_session(db_path: str, user_id: int, ttl_days: int = 7) -> str:
    """Generate a secure session token, persist it, and return the token string."""
...
def get_session_user(db_path: str, token: str) -> Optional[dict]:
    """Return the user dict for a valid, unexpired session token, or None."""
    now = datetime.now(timezone.utc).isoformat()
    with sqlite3.connect(db_path) as conn:
        conn.row_factory = sqlite3.Row
